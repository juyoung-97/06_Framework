...
function getTotalCount(){
    ...
    .then((result) => { // 첫번째 콜백함수가 완료된 후 호출되는 콜백함수
        ...
        // 어떤식으로 처리할지 정의
        // -> #totalCount 인 span 태그의 내용으로 result 값 삽입
...
|| todoContent.value.trim().length === 0){
    ...
    // - 요청 주소 : "/ajax/add"
    // - 데이터 전달방식 : POST
    // - 전달 데이터(파라미터) : todoTitle값, todoContent 값
    // JS <-> Java
    ...
    */
    ...
    // todoTitle 과 todoContent 를 저장한 JS 객체
    const param = {
        // key : value
    ...
    fetch("/ajax/add", {
        ...
        // 요청 본문 : param 이라는 JS객체를 JSON(string)으로 변환
...
// popuplayer 의 제목 클릭 시 popuplayer 보이게 하기



// popuplayer 의 X 버튼 클릭 시 popuplayer 숨기기
